...
class ProducerLiteConfig:
    ...
    # V3 4P safety scoring. This is deliberately a soft score penalty, not a
    # launch veto, so the original Intervention tempo and full-drain options
    # stay available when the planner sees enough ROI.
    enable_ffa_v3_safety: bool = False
    v3_far_enemy_dist: float = 45.0
    v3_far_enemy_prod_max: float = 2.0
    v3_far_enemy_penalty: float = 3.5
    v3_far_enemy_dist_penalty: float = 0.04
    v3_low_value_neutral_dist: float = 55.0
    v3_low_value_neutral_prod_max: float = 1.0
    v3_low_value_neutral_penalty: float = 2.0
...
def _apply_ffa_v3_safety_penalty(
    score: Tensor,
    *,
    obs,
    cache,
    ...
    """Softly demote the two failure modes seen in Alyce 4P replay review.

    Non-winning replay clusters often spent early tempo on far low-value
    neutrals and midgame tempo on fragmented far/low-value enemy attacks. This
    does not forbid those moves; it only makes them clear a higher score bar
    unless they are high-production or hit the current leader.
    """
    if not bool(config.enable_ffa_v3_safety) or int(player_count) < 4:
        return score

    P = int(obs.P)
    if P <= 0 or int(cand_valid.numel()) == 0:
        return score

    flat_src = cand_src.reshape(-1).clamp(min=0, max=max(P - 1, 0)).long()
    flat_tgt_short = cand_tgt_short.reshape(-1)
    flat_tgt = target_idx[flat_tgt_short].clamp(min=0, max=max(P - 1, 0)).long()
    immediate_dist = cache.cross_dist[0].to(dtype)[flat_src, flat_tgt]

    target_planets = target_idx.clamp(0, P - 1)
    owner_abs = obs.owner_abs.to(torch.long)
    target_owner_abs = owner_abs[target_planets]
    target_owner = target_owner_abs.clamp(min=0, max=max(int(player_count) - 1, 0))
    target_is_enemy = (
        target_exists
        & (target_owner_abs >= 0)
        & (target_owner_abs != int(pid))
        & (target_owner_abs < int(player_count))
    )

    strength = _compute_strength(obs, prod, int(player_count), dtype, device)
    leader = int(torch.argmax(strength).detach().item()) if int(player_count) > 0 else int(pid)
    target_is_leader = target_is_enemy & (target_owner == leader) & (leader != int(pid))

    enemy_low_value_short = (
        target_is_enemy
        & ~target_is_leader
        & (target_prod <= float(config.v3_far_enemy_prod_max))
    )
    enemy_low_value = enemy_low_value_short[flat_tgt_short] & ~cand_is_def.reshape(-1)
    enemy_far = immediate_dist >= float(config.v3_far_enemy_dist)
    enemy_dist_penalty = (
        float(config.v3_far_enemy_penalty)
        + (immediate_dist - float(config.v3_far_enemy_dist)).clamp(min=0.0)
        * float(config.v3_far_enemy_dist_penalty)
    )

    neutral_low_value_short = (
        obs.is_neutral[target_planets]
        & (target_prod <= float(config.v3_low_value_neutral_prod_max))
    )
    neutral_low_value = neutral_low_value_short[flat_tgt_short] & ~cand_is_def.reshape(-1)
    neutral_far = immediate_dist >= float(config.v3_low_value_neutral_dist)

    valid_flat = cand_valid.reshape(-1)
    penalty = torch.zeros_like(score)
    penalty = penalty + torch.where(
        enemy_low_value & enemy_far & valid_flat,
        enemy_dist_penalty,
        torch.zeros_like(score),
    )
    penalty = penalty + torch.where(
        neutral_low_value & neutral_far & valid_flat,
        torch.full_like(score, float(config.v3_low_value_neutral_penalty)),
        torch.zeros_like(score),
    )
...
def plan_lite_waves(
    *,
    ...
    score = _apply_ffa_v3_safety_penalty(
        score,
        obs=obs,
        cache=cache,
        prod=prod,
